...
const Rent: React.FC = () => {
  ...
  const handleContact = (e: React.FormEvent, selectedProperty: Property) => {
    e.preventDefault();

    if (!isAuthenticated) {
      setProperty(selectedProperty); // Save which property user clicked
  ...
  useEffect(() => {
    if (isAuthenticated && pendingAction === 'contact') {
      setShowSignUpModal(false); // Hide sign up modal
      setShowModal(true);        // Show contact modal
  ...
  return (
    ...
    <div className='bg-gray-100 '>
      <motion.div
        className="p-5 w-[46rem] sm:w-[58rem] md:w-[70rem] lg:w-[64rem] xl:w-full bg-gray-100 "
